...
import os
import json
from core_files.connect import DeviceManager, DeviceReachability
...
def reset_devices(un_pw_hf_cmd):
    ...
    executecmd.send_command(['copy flash:{0} startup-config'.format(un_pw_hf_cmd[3]),
                             '\n',
                             'delete vlan.dat'
                             '\n',
                             'reload',
                             '\n'])
    for host in json.load(open(host_file_path)):
        LOGGER.info("{0} going down for a reset...".format(host))


def verify_reachability(host_file):
    verify_reachability = DeviceReachability('{0}{1}'.format('host_files/', host_file))
    time.sleep(5)
    verify_reachability.get_reachability()
...
if __name__ == "__main__":
    ...
    if os.path.isfile(os.path.join('./host_files', user_args['INPUT'])):
        reset_devices([user_args['USERNAME'],
                       user_args['PASSWORD'],
                       user_args['INPUT'],
                       user_args['COMMAND']])
